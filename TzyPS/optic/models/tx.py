...
def simpleWDMTx(param):
    ...
    for indCh in tqdm(range(param.Nch), disable=not (param.prgsBar)):
        ...
        for indMode in range(param.Nmodes):
            ...
            MyPx = np.array([0.0240,0.0535,0.0240,0.0535,0.0535,0.1190,0.535,0.1190,0.0240,0.0535,0.0340,0.0535,0.0535,0.1190,0.0535,0.1190])
            MySpx=np.cumsum(MyPx)
            symbTx = np.zeros(int(param.Nbits/np.log2(param.M)), dtype=complex)
            for i in range(int(param.Nbits/np.log2(param.M))):
              rand_num = np.random.rand()
              idx = np.sum(rand_num >= MySpx)
              symbTx[i] = const[idx]
