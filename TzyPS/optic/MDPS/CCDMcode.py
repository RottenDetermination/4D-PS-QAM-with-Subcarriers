...
def CCDMEnc(M,n,Block_Length,Block_Count,Prob_n_i):
    ...
    lib = ctypes.cdll.LoadLibrary('d:/visual studio/CCDM/x64/Debug/CCDMEnc.dll')
    ...
    for i in range(0,Block_Count):
        ...
        Matrix_to_csv(bitsQ,'EncPy.csv')
        lib.CCDM_Enc(Block_Length,n,ProbCount)
        ResQ = csv_to_Matrix("d:/visual studio/EncCpp.csv")
        # Get sign
        for j in range(0,n):
            for k in range(0,Amp_Count):
                if(ResQ[j] == k+1):
...
def CCDMDec(M,n,Block_Length,Block_Count,Prob_n_i,Sym):
    # Calculate some parameters
    lib = ctypes.cdll.LoadLibrary('d:/visual studio/CCDM/x64/Debug/CCDMDec.dll')
        ...
        for j in range(0,n):
            for k in range(0,Amp_Count):
                if((Amp_I[h][j] == Amp_Data[k]).all()):
                    Amp_I[h][j] = k
        Matrix_to_csv(Amp_I[h,:],'DecPy.csv')
        lib.CCDM_Dec(n,ProbCount,Block_Length)
        BitI = csv_to_Matrix("d:/visual studio/DecCpp.csv")
        ...
        for j in range(0,n):
            for k in range(0,Amp_Count):
                if((Amp_Q[i][j] == Amp_Data[k]).all()):
                    Amp_Q[i][j] = k
        Matrix_to_csv(Amp_Q[i,:],'DecPy.csv')
        lib.CCDM_Dec(n,ProbCount,Block_Length)
        BitQ = csv_to_Matrix("d:/visual studio/DecCpp.csv")
        for m in range(0,Block_Length):
            ResQ[i][m] = BitQ[m]
        print(f'{i+1}/{Block_Count}',end='\r')
    print("")
